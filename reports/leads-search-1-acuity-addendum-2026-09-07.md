...
~~One open item: the /contact form fired generate_lead for 27 paid-search sessions...~~ **SUPERSEDED 2026-09-08.** `generate_lead` is not a form submission at all. It fires on page load of `/contact` — the ratio to `page_view` on that path is exactly 1.00 across all 170 events in all five months it has existed, and `form_submit` on `/contact` is 0. There is nothing to check in the inbox and nothing to import; the event never measured a submission. See `data/supermetrics-archive-2026-09-08/NOTES-ga4-events.md`.
